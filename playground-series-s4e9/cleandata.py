...
df['hp'] = df.groupby(['fuel_encoded', pd.cut(df['year_scaled'], bins=10)], observed=False)['hp'].transform(lambda x: x.fillna(x.mean()))
df['tank_size'] = df.groupby(['fuel_encoded', pd.cut(df['year_scaled'], bins=10), pd.cut(df['hp'], bins=10)], observed=False)['tank_size'].transform(lambda x: x.fillna(x.mean()))
df['cyl'] = df.groupby([pd.cut(df['hp'], bins=10)], observed=False)['cyl'].transform(lambda x: x.fillna(x.mean()))
